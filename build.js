...
const ABOUT_CONTENT = {
  en: {
    ...
    sections: [
      ...
      { h: 'Who\'s behind this?', p: 'DateCalc is built by a small team in France. Our lead developer Mike obsesses over edge cases — like whether Japanese week numbers should start on Sunday or Monday (spoiler: it depends on whether you follow the ISO standard, and yes, we implement both), how to handle February 29 birthdays in a non-leap year, or whether the Korean traditional age system should default "on" for Korean users (we added a clear toggle). Getting the details right across 10 languages is roughly 80% of the work.' },
  ...
  fr: {
    ...
    sections: [
      ...
      { h: 'Qui est derrière ?', p: 'DateCalc est développé par une petite équipe en France. Notre dev principal Mike est obsédé par les cas limites — du genre savoir si les numéros de semaine japonais doivent commencer le dimanche ou le lundi (spoiler : ça dépend du respect de la norme ISO, et on implémente les deux), comment gérer les anniversaires du 29 février en année non bissextile, ou si l\'âge coréen traditionnel doit être activé par défaut pour les utilisateurs coréens (on a ajouté un toggle clair). Soigner les détails dans 10 langues, c\'est environ 80 % du boulot.' },
  ...
  es: {
    ...
    sections: [
      ...
      { h: '¿Quién está detrás?', p: 'DateCalc lo construye un pequeño equipo en Francia. Nuestro desarrollador principal Mike se obsesiona con los casos límite — como si los números de semana japoneses deben empezar en domingo o lunes (spoiler: depende de si sigues el estándar ISO, e implementamos los dos), cómo manejar los cumpleaños del 29 de febrero en años no bisiestos, o si la edad coreana tradicional debe activarse por defecto para usuarios coreanos (añadimos un toggle claro). Acertar en los detalles a través de 10 idiomas es aproximadamente el 80 % del trabajo.' },
  ...
  pt: {
    ...
    sections: [
      ...
      { h: 'Quem está por trás disso?', p: 'DateCalc é construído por uma pequena equipe na França. Nosso desenvolvedor principal Mike é obcecado por casos limite — como saber se os números de semana japoneses devem começar no domingo ou na segunda (spoiler: depende se você segue o padrão ISO, e implementamos os dois), como lidar com aniversários de 29 de fevereiro em ano não bissexto, ou se a idade tradicional coreana deve ser ativada por padrão para usuários coreanos (adicionamos um toggle claro). Acertar os detalhes em 10 idiomas é cerca de 80% do trabalho.' },
  ...
  de: {
    ...
    sections: [
      ...
      { h: 'Wer steckt dahinter?', p: 'DateCalc wird von einem kleinen Team in Frankreich entwickelt. Unser leitender Entwickler Mike ist besessen von Grenzfällen — etwa, ob japanische Wochennummern am Sonntag oder Montag beginnen sollten (Spoiler: kommt drauf an, ob du der ISO-Norm folgst, und wir implementieren beides), wie Geburtstage am 29. Februar in einem Nicht-Schaltjahr zu behandeln sind, oder ob das traditionelle koreanische Alter für koreanische Nutzer standardmäßig an sein sollte (wir haben einen klaren Schalter hinzugefügt). Die Details über 10 Sprachen hinweg richtig hinzukriegen, macht ungefähr 80 % der Arbeit aus.' },
  ...
  it: {
    ...
    sections: [
      ...
      { h: 'Chi c\'è dietro?', p: 'DateCalc è costruito da un piccolo team in Francia. Il nostro sviluppatore principale Mike è ossessionato dai casi limite — tipo se i numeri di settimana giapponesi debbano iniziare la domenica o il lunedì (spoiler: dipende se segui lo standard ISO, e implementiamo entrambi), come gestire i compleanni del 29 febbraio in un anno non bisestile, o se l\'età coreana tradizionale debba essere attivata di default per gli utenti coreani (abbiamo aggiunto un toggle chiaro). Fare bene i dettagli in 10 lingue è circa l\'80% del lavoro.' },
  ...
  pl: {
    ...
    sections: [
      ...
      { h: 'Kto za tym stoi?', p: 'DateCalc budowany jest przez mały zespół we Francji. Nasz główny programista Mike ma obsesję na punkcie przypadków brzegowych — np. czy japońskie numery tygodni powinny zaczynać się od niedzieli czy od poniedziałku (spoiler: zależy od tego, czy trzymasz się standardu ISO, a implementujemy oba), jak obsłużyć urodziny 29 lutego w roku nieprzestępnym, czy tradycyjny koreański system wieku powinien być domyślnie włączony dla koreańskich użytkowników (dodaliśmy jasny przełącznik). Dopilnowanie detali w 10 językach to jakieś 80% pracy.' },
  ...
  ja: {
    ...
    h1: 'DateCalc.appについて',
    intro: 'DateCalc.appはフランスを拠点とする2人チームの小さな独立プロジェクトです。私たち自身が使いたくなるような日付・時刻計算機 — クリーンで速く、ダークパターンがなく、他サイトが間違えるエッジケースまで正確な — を作っています。サイトはGoogle AdSenseにより無料で維持されています。この最小限の広告層を除けば、アカウント、ニュースレター、アップセルは一切ありません。日本語については、日本のカレンダー慣習への敬意を込めて、週番号の開始曜日や和暦表記など細部まで丁寧に対応しています。',
    sections: [
      ...
      { h: '制作者について', p: 'DateCalcはフランスの小さなチームによって作られています。リード開発者のMikeはエッジケースにこだわります — 例えば日本の週番号が日曜始まりか月曜始まりか（ネタバレ：ISO標準に従うかどうかで変わるので両方実装しました）、閏年でない年の2月29日生まれの扱い、韓国の伝統的な年齢計算を韓国ユーザーにデフォルトでオンにすべきか（明確なトグルを追加しました）といった話です。10言語全体で細部を正しく整えることが仕事の約8割を占めます。' },
  ...
  ko: {
    ...
    h1: 'DateCalc.app 소개',
    intro: 'DateCalc.app은 프랑스에 기반을 둔 두 명의 팀이 만드는 작은 독립 프로젝트입니다. 우리 자신이 쓰고 싶은 종류의 날짜·시간 계산기 — 깔끔하고 빠르고, 다크 패턴이 없고, 다른 사이트가 틀리는 경계 케이스까지 정확한 것 — 을 만듭니다. 사이트는 Google AdSense로 운영되어 무료로 유지되며, 그 최소한의 광고 레이어를 제외하면 계정, 뉴스레터, 업셀이 일절 없습니다. 한국어 버전에서는 만 나이와 한국식 전통 나이를 모두 지원하는 등, 한국의 관습을 존중한 세부 구현에 특별히 신경 씁니다.',
    sections: [
      ...
      { h: '제작자', p: 'DateCalc은 프랑스의 작은 팀이 만듭니다. 리드 개발자 Mike는 경계 케이스에 집착합니다 — 예를 들어 일본 주 번호가 일요일 시작인지 월요일 시작인지(스포일러: ISO 표준을 따르는지에 달려 있고, 둘 다 구현했습니다), 윤년이 아닌 해의 2월 29일 생일 처리, 한국식 전통 나이 계산을 한국 사용자에게 기본 활성화할지(명확한 토글을 추가했습니다) 같은 문제들입니다. 10개 언어에 걸쳐 세부 사항을 제대로 맞추는 것이 일의 약 80%를 차지합니다.' },
  ...
  nl: {
    ...
    sections: [
      ...
      { h: 'Wie zit erachter?', p: 'DateCalc wordt gebouwd door een klein team in Frankrijk. Onze lead developer Mike is geobsedeerd door edge cases — zoals of Japanse weeknummers op zondag of op maandag moeten beginnen (spoiler: hangt ervan af of je de ISO-standaard volgt, en we implementeren beide), hoe je met 29-februari-verjaardagen in een niet-schrikkeljaar omgaat, of of de traditionele Koreaanse leeftijd standaard aan moet staan voor Koreaanse gebruikers (we hebben een duidelijke toggle toegevoegd). De details goed krijgen in 10 talen is zo\'n 80% van het werk.' },
