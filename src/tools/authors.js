...
const BIO = {
  ...
  fr: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Développeur solo de DateCalc.app.',
    bio: '<p>Mike est le développeur derrière DateCalc.app. Basé en France, obsédé par les fuseaux horaires, les cas limites des dates et le fait de faire fonctionner les outils de la même façon dans 10 langues. A des opinions tranchées sur la question de savoir si les semaines commencent le dimanche ou le lundi (réponse : lundi, ISO 8601, fin du débat), sur ce qui détermine à quelle date « appartient » un jour (minuit UTC ? minuit local ? le téléphone de l\'utilisateur ?), et sur le fait que les secondes intercalaires méritent un calculateur (pas encore).</p><p>Avant ça il a livré des outils dans de l\'ERP et deux apps grand public dont plus personne ne parle. DateCalc est le projet qu\'il a continué à écrire le soir jusqu\'à ce qu\'il dépasse le stade du side project. C\'est encore une opération à une seule personne : tout ce que vous voyez — la copie dans 10 langues, les cas limites, les choix typographiques — c\'est Mike.</p><p>Un bug, une traduction foireuse, un cas limite qu\'on a raté ? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
  ...
  es: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Desarrollador en solitario de DateCalc.app.',
    bio: '<p>Mike es el desarrollador detrás de DateCalc.app. Con base en Francia, obsesionado con zonas horarias, casos límite de fechas y hacer que cada cálculo encaje a la primera. Tiene opiniones firmes sobre si las semanas empiezan en domingo o lunes (respuesta: lunes, ISO 8601, fin del debate), sobre a qué fecha "pertenece" un día (¿medianoche UTC? ¿medianoche local? ¿la del teléfono del usuario?) y sobre si los segundos intercalares merecen su propia calculadora (todavía no).</p><p>Antes de esto entregó herramientas en ERP empresarial y un par de apps de consumo de las que ya nadie habla. DateCalc es el proyecto que siguió escribiendo por las noches hasta que se le fue de las manos. Sigue siendo una operación de una sola persona: todo lo que ves — los textos, los casos límite, las elecciones tipográficas — es Mike.</p><p>¿Un bug, una errata o un caso límite que se nos escapó? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
  ...
  pt: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Desenvolvedor solo do DateCalc.app.',
    bio: '<p>Mike é o desenvolvedor por trás do DateCalc.app. Baseado na França, obcecado por fusos horários, casos limite de datas e fazer com que as ferramentas funcionem exatamente igual em 10 idiomas. Tem opiniões firmes sobre se as semanas começam no domingo ou na segunda (resposta: segunda, ISO 8601, fim do debate), sobre a que data um dia "pertence" (meia-noite UTC? meia-noite local? o telefone do usuário?) e sobre se os segundos intercalares merecem uma calculadora própria (ainda não).</p><p>Antes disso entregou ferramentas em ERP corporativo e algumas apps de consumo de que ninguém mais fala. DateCalc é o projeto que ele continuou escrevendo à noite até virar algo maior do que um side project. Ainda é uma operação de uma pessoa só: tudo o que você vê — a cópia em 10 idiomas, os casos limite, as escolhas tipográficas — é o Mike.</p><p>Um bug, uma tradução errada, um caso limite que passou? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
  ...
  de: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Solo-Entwickler von DateCalc.app.',
    bio: '<p>Mike ist der Entwickler hinter DateCalc.app. Wohnhaft in Frankreich, besessen von Zeitzonen, Grenzfällen bei Datumsberechnungen und der Idee, dass die Tools in 10 Sprachen genau gleich funktionieren müssen. Hat feste Meinungen dazu, ob Wochen am Sonntag oder Montag beginnen (Antwort: Montag, ISO 8601, Ende der Debatte), zu welchem Datum ein Tag "gehört" (Mitternacht UTC? lokale Mitternacht? das Telefon des Nutzers?) und ob Schaltsekunden einen eigenen Rechner verdienen (noch nicht).</p><p>Davor hat er Tools im Enterprise-ERP-Bereich und ein paar Consumer-Apps abgeliefert, über die niemand mehr spricht. DateCalc ist das Projekt, das er abends immer weiter geschrieben hat, bis es kein Nebenprojekt mehr war. Es ist immer noch ein Ein-Personen-Betrieb: Alles, was du siehst — der Text in 10 Sprachen, die Grenzfälle, die Typografie-Entscheidungen — ist Mike.</p><p>Bug gefunden, schlechte Übersetzung, einen Grenzfall übersehen? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
  ...
  pl: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Solo developer DateCalc.app.',
    bio: '<p>Mike to programista stojący za DateCalc.app. Mieszka we Francji, ma obsesję na punkcie stref czasowych, przypadków brzegowych dat i tego, żeby narzędzia działały dokładnie tak samo w 10 językach. Ma mocne opinie na temat tego, czy tydzień zaczyna się w niedzielę czy w poniedziałek (odpowiedź: poniedziałek, ISO 8601, koniec dyskusji), do której daty "należy" dany dzień (północ UTC? lokalna północ? ta z telefonu użytkownika?) i czy sekundy przestępne zasługują na własny kalkulator (jeszcze nie).</p><p>Wcześniej dostarczał narzędzia w korporacyjnym ERP i kilka aplikacji konsumenckich, o których nikt już nie pamięta. DateCalc to projekt, który pisał wieczorami tak długo, aż przestał być pobocznym projektem. To nadal przedsięwzięcie jednoosobowe: wszystko, co widzisz — tekst w 10 językach, przypadki brzegowe, decyzje typograficzne — to Mike.</p><p>Znalazłeś buga, złe tłumaczenie albo przypadek brzegowy, którego nie obsłużyliśmy? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
  ...
  ja: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'DateCalc.appを一人で開発・運営。',
    bio: '<p>MikeはDateCalc.appの開発者です。フランス在住。タイムゾーン、日付のエッジケース、そして10言語すべてでツールがまったく同じように動くことに執着しています。週が日曜始まりか月曜始まりか（答え：月曜、ISO 8601、議論終了）、ある「日」がどの日付に属するのか（UTCの深夜？ローカルの深夜？ユーザーの端末の？）、そして閏秒に専用計算機が必要か（まだ必要ない）について、強い意見を持っています。現在のツールはISO 8601の月曜始まりがデフォルトで、日本式（日曜始まり・1月1日を含む週を第1週とする方式）の切替はロードマップに載せています。</p><p>DateCalcの前はエンタープライズERP向けのツールをいくつか作り、その後誰も話題にしない消費者向けアプリも作っていました。DateCalcは、夜に書き続けていたら副業プロジェクトの枠を超えてしまった、そんなプロジェクトです。今もなお個人運営で、10言語の文章、エッジケース、タイポグラフィの選択まですべてMike一人が担当しています。</p><p>バグ、翻訳の間違い、見落とされたエッジケースを見つけたら、<a href="mailto:contact@datecalc.app">contact@datecalc.app</a> までお知らせください。</p>',
  ...
  ko: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'DateCalc.app을 1인으로 운영하는 개발자.',
    bio: '<p>Mike는 DateCalc.app을 만든 개발자입니다. 프랑스 거주. 시간대, 날짜의 경계 케이스, 그리고 10개 언어 모두에서 도구가 정확히 똑같이 동작하는 것에 집착합니다. 한 주가 일요일에 시작하는지 월요일에 시작하는지(답: 월요일, ISO 8601, 논쟁 종료), 어떤 날이 어느 날짜에 "속하는지"(UTC 자정? 로컬 자정? 사용자 전화기의 자정?), 윤초가 전용 계산기를 가질 만한지(아직은 아니오)에 대해 강한 의견이 있습니다. 현재 계산기는 만 나이를 기본으로 하며, 한국식 전통 나이 전용 모드와 YYYY.MM.DD 표기는 로드맵에 올려 둔 상태입니다.</p><p>그 전에는 기업용 ERP 도구와 이제 아무도 기억하지 못하는 소비자 앱 몇 개를 만들었습니다. DateCalc은 저녁마다 계속 써내려가다가 사이드 프로젝트의 범주를 넘어선 결과물입니다. 여전히 1인 운영이며, 10개 언어의 문구, 경계 케이스, 타이포그래피 선택까지 전부 Mike가 맡고 있습니다.</p><p>버그, 잘못된 번역, 놓친 경계 케이스를 발견하셨다면 <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>로 연락 주십시오.</p>',
  ...
  nl: {
    title: 'Mike — DateCalc.app',
    h1: 'Mike',
    lead: 'Solo-ontwikkelaar van DateCalc.app.',
    bio: '<p>Mike is de ontwikkelaar achter DateCalc.app. Woont in Frankrijk, geobsedeerd door tijdzones, edge cases van datums en het idee dat de tools in 10 talen exact hetzelfde moeten werken. Heeft uitgesproken meningen over de vraag of weken op zondag of maandag beginnen (antwoord: maandag, ISO 8601, einde discussie), bij welke datum een dag "hoort" (middernacht UTC? lokale middernacht? die van de telefoon van de gebruiker?), en of schrikkelseconden een eigen calculator verdienen (nog niet).</p><p>Daarvoor bouwde hij tools in enterprise-ERP en een paar consumenten-apps waar niemand meer over praat. DateCalc is het project dat hij \'s avonds bleef schrijven tot het het label "sideproject" ontgroeide. Het is nog steeds een eenmanszaak: alles wat je ziet — de copy in 10 talen, de edge cases, de typografische keuzes — is Mike.</p><p>Bug gevonden, slechte vertaling, een edge case gemist? <a href="mailto:contact@datecalc.app">contact@datecalc.app</a>.</p>',
